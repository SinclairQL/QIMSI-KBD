; *************************************
; QIMSI keyboard driver for Minerva ROM
; *************************************
; Based on Q40/Q60/Q68 code by Richard Zidlicky
;
; Modifications for QIMSI by Jan Bredenbeek
; Use and distribution governed by GNU Public License v3
; 
; Changelog:
;
; 20231008 v1.0 (JB)    initial version for QIMSI, taken from Q68 code
...
VAR.KBPS2    EQU        $37     ; (byte) <>0 when PS/2 keyboard enabled
VAR.KBenc    EQU        $38     ; (long) original Minerva sx_kbenc pointer
...
kbd_init:
	...
	movem.l	initreg,-(a7)
        lea     signmsg,a1
        bsr     initmsg
        cmpi.l  #'QIMS',$C00C   ; check for presence of QIMSI
        bne.s   ins_chk         ; oops...
	moveq	#0,d0
	trap	#1
...
;* VAR.LEN should be enough
	move.l	#VAR.LEN,d1     ; length
	moveq	#$18,d0		;  MT.ALCHP
	trap	#1		; allocate space
	tst.l	d0
	bne.s	ROM_EXIT 	; exit if error
	move.l	a0,a3           ; base of linkage block
        lea     procs(pc),a1    ; BASIC procs
        move.w  bp.init,a2
        jsr     (a2)            ; link in procs
        move.l  sx_kbenc(a4),VAR.KBenc(a3) ; save ptr to original kbenc routine
        lea	L01_KTAB(pc),a0 ; default to US
...
;;	lea	POLL_SERver(pc),a1 ; redundant code, use real address
        tst.b   KEY_UNLOCK      ; keyboard may start up
...
signmsg string$ {'QIMSI external keyboard driver for Minerva v[version]',10}
signms2 string$ {'Enter KBD_PS2 to enable, KBD_QL to disable',10}

inerrms	string$	{'Incompatible QDOS Version (are you REALLY running Minerva?)',10}
no_qims string$ {'QIMSI not detected!',10}
...
procs   dc.w    4               ; three procedures but allow for long names
        dc.w    kbd_ps2-*
        dc.b    7,'KBD_PS2'
        dc.w    kbd_ql-*
        dc.b    6,'KBD_QL'
        dc.w    kbtable-*
        dc.b    13,'KBD_TABLE_PS2'
        dc.w    0               ; end of procs
...
*       A4 points to sx_base (Minerva extended sysvars)
...
kbd_ps2:
        ...
        bne.s   kbd_rts
        lea     Q68kbenc(pc),a2
        move.l  a2,sx_kbenc(a4) ; set our keyboard decoding routine
...
kbd_ql:
        bsr     get_lb
        bne     kbd_rts
        sf      VAR.KBPS2(a3)   ; reset flag
        move.l  VAR.KBenc(a3),sx_kbenc(a4) ; restore Minerva keyboard handler
        lea     VAR.IPClnk(a3),a0
        lea     sx_ipcom(a4),a1
        tst.l   (a1)
        beq.s   kbd_rts         ; don't unlink if there is no list at all!
        move.w  $d4,a2          ; UT.UNLINK
        jmp     (a2)            ; exit

* KBD_TABLE_PS2 command sets the keyboard table
* Usage: KBTABLE_PS2 <country code>
...
RDKEYB:                         ; polled interrupt entry
        tst.b   VAR.KBPS2(a3)   ; PS/2 keyboard enabled?
        beq.s   no_ps2          ; no, return immediately!
	movem.l	rkeyreg,-(a7)
...
kbl	move.b	KEY_CODE,d0	; get scan code
	tst.b	KEY_UNLOCK	; signal 'got it'
